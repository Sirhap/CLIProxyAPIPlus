...
func windsurfStripGRPCFrame(frame []byte) ([]byte, error) {
	if len(frame) < 5 {
		return frame, nil
	}
	if frame[0] != 0 {
		return nil, fmt.Errorf("windsurf grpc: compressed frames are not supported")
	}
	size := int(binary.BigEndian.Uint32(frame[1:5]))
	if size < 0 || len(frame) < 5+size {
		return nil, fmt.Errorf("windsurf grpc: incomplete frame")
	}
	return frame[5 : 5+size], nil
}
...
func windsurfExtractGRPCFrames(buf []byte) ([][]byte, error) {
	...
	for offset+5 <= len(buf) {
		...
		frames = append(frames, buf[offset+5:offset+5+size])
		offset += 5 + size
	}
	if offset != len(buf) {
...
func (c *windsurfGRPCClient) Unary(ctx context.Context, path string, payload []byte, timeout time.Duration) ([]byte, error) {
	...
	frames, err := windsurfExtractGRPCFrames(body)
	if err != nil {
		if stripped, stripErr := windsurfStripGRPCFrame(body); stripErr == nil {
			return stripped, nil
		}
		return nil, err
	}
